...
export function useTasks() {
  ...
  const linkTask = useCallback(
    async (id: string, kind: TaskLinkKind, value: string): Promise<boolean> => {
      const next = tasks.map((task) =>
        task.id === id ? toggleTaskLink(task, kind, value) : task,
      );
      return save(next);
    },
    [save, tasks],
  ...
  const renameColumn = useCallback(
    async (id: string, title: string): Promise<boolean> => {
      ...
      return save(
        tasks,
        columns.map((column) =>
          column.id === id ? { ...column, title: title.trim() } : column,
        ),
      );
    },
    [columns, save, tasks],
  ...
  const toggleColumn = useCallback(
    async (id: string): Promise<boolean> => {
      return save(
        tasks,
        columns.map((column) =>
          column.id === id ? toggleColumnVisibility(column) : column,
        ),
      );
    },
    [columns, save, tasks],
  ...
  const deleteColumn = useCallback(
    async (id: string): Promise<boolean> => {
      ...
      const nextTasks = tasks.map((task) =>
        task.status === id
          ? { ...task, status: fallback, updatedAt: new Date().toISOString() }
          : task,
      );
